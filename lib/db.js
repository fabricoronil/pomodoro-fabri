...
function rowToSettings(row) {
  ...
  // la columna numeric vuelve como string desde postgrest
...
function settingsToRow(s) {
  const row = {};
  for (const [key, col] of Object.entries(SETTINGS_COLUMNS)) row[col] = s[key];
...
export async function fetchSettings() {
  ...
  if (!data) {
    await supabase
      .from("user_settings")
      .upsert({ user_id: CURRENT_USER_ID, ...settingsToRow(local) }, { onConflict: "user_id" });
    return local;
  }
